...
import { type Transaction, type PendingTx, type WalletProps } from "./types";
import {
  downloadFile,
...
export const Wallet = ({ privateKey, publicKey, clearKeys }: WalletProps) => {
  ...
  const formatted = (balance / 100000).toLocaleString("en-US", {
  ...
  const [txn, setTxn] = useState<Transaction>({ ...baseTxn });
    ...
    <div className="p-6 max-w-md mx-auto bg-base-100 shadow-xl rounded-2xl">
      ...
      <div className="mb-6 p-4 bg-base-200 rounded-xl shadow-inner space-y-3">
        ...
        <input
          ...
          value={txn.amount || ""}
          onChange={(e) =>
            mutateTransactionField("amount", parseInt(e.target.value) || 0)
          }
